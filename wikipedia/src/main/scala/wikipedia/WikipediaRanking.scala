...
object WikipediaRanking extends WikipediaRankingInterface :
  ...
  // setMaster("local[K]") : Run Spark locally with K worker threads
  val conf: SparkConf = new SparkConf().setAppName("Wikipedia").setMaster("local[6]")
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.cache()
    // sort by value of a tuple: sortBy(_._2)
    // sort descending: sortBy(-<value>) (add minus sign)
